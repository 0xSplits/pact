...
export interface Pact {
  ...
  totalTokens: number;
  holders: Array<{ address: string; beforePct: number; afterPct: number; tokens: number; delivery: string }>;
...
export const tokensBetween = (pact: Pact, R0: number, R1: number) => (fractionAt(pact, R1) - fractionAt(pact, R0)) * pact.totalTokens;
...
export function costForUnits(curve: CurveParams | null | undefined, sold: number, units: number): number {
...
export function unitsForBudget(curve: CurveParams, sold: number, remaining: number, budget: number): number {
  let units = 0;
...
export function valuationForUnitIndex(curve: CurveParams | null | undefined, unitIndex: number, totalTokens: number): number {
  if (!curve) return 0;
