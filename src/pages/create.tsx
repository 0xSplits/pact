...
const oneDecimal = (v: string | number) => (Math.round((Number(v) || 0) * 10) / 10).toFixed(1);
...
function integerCurveMaxUsd(form: CreateForm) {
  ...
  const curve = deriveOfferingCurve({
    valuation: { floor: Math.round(cap * (1 - spread)), ceiling: Math.round(cap * (1 + spread)) },
  ...
  switch (key) {
    ...
    case 'raiseMin': {
      ...
      if (sellableMax != null && rmin > sellableMax) {
        return 'Minimum exceeds the sellable maximum of $' + Math.floor(sellableMax).toLocaleString('en-US') + '.';
      }
      return '';
    }
    case 'raiseMax':
...
function formIsValid(form: CreateForm, holders: HolderRow[], d: ReturnType<typeof deriveCurve>) {
...
function buildPact(form: CreateForm, holders: HolderRow[], wallet: string): Pact {
  const rmin = parseMoney(form.raiseMin);
  const rmax = parseMoney(form.raiseMax);
  const dilution = (+form.dilution || 0) / 100;
  const spread = (+form.spread || 0) / 100;
  ...
  return {
    ...
    publicUnits: Math.round(Math.round(dilution * TOTAL_SHARES) * ((+form.publicPct || 0) / 100)),
  };
}
