...
[Offering contract](../contracts/docs/contracts.md) is the whole backend.
This guide covers the calls every integration needs; the contract
specification holds the exact semantics, and the
[architecture](architecture.md) the vocabulary.

Agents with a shell: the [`skills/pact`](../skills/pact/SKILL.md) skill
turns this guide into `cast` commands (`npx skills add 0xSplits/pact`).
...
`offering` is the id the app routes take.
...
`src/lib/chain/voucher.ts`; the skill's `references/vouchers.md` walks
both sides with `cast`.
