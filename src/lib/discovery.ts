...
export interface DiscoveryInputs {
  ...
  // The published CLI package (`@splits/pact`).
  cliPackage: string;
  repository: string;
  origin?: string;
}
...
export function wellKnownManifest({
  ...
  return {
    ...
    cli: {
      npm: cliPackage,
      run: `npx ${cliPackage}`,
      skills: `npx ${cliPackage} skills add`,
    },
    mcp: {
      registryName: "org.splits/pact-mcp",
      npm: cliPackage,
      transport: "stdio",
      run: `npx ${cliPackage} --mcp`,
    },
    skill: `npx ${cliPackage} skills add`,
  };
}
...
- CLI: \`npx ${cliPackage} --help\`; every read and write, unsigned transactions by default, \`--format json\` when relaying to a signer
- Skills: \`npx ${cliPackage} skills add\` installs the knowledge + command skills into every detected agent harness
- MCP (stdio, for shell-less hosts): \`npx -y ${cliPackage}@latest --mcp\`
