...
import { chunkRanges, cachedScan, seedOffering, listOfferings, findOffering, SCAN_CHUNK_BLOCKS } from './offerings.ts';

const fakeStorage = () => {
  const map = new Map<string, string>();
  return { getItem: (k: string) => (map.has(k) ? map.get(k)! : null), setItem: (k: string, v: string) => void map.set(k, v), map };
};
...
function fakeGetLogs(logsByFromBlock: Record<number, any[]> = {}) {
  ...
  const fn = async ({ fromBlock, toBlock }: { fromBlock: number; toBlock: number }) => {
...
test('chunkRanges splits into 10k chunks with exact edges', () => {
  assert.deepEqual(chunkRanges(0, 25000), [[0, 9999], [10000, 19999], [20000, 25000]]);
...
test('cachedScan cold-scans from the deploy block in chunks', async () => {
  ...
  assert.deepEqual(getLogs.ranges, [[100, 10099], [10100, 20099], [20100, 25099]]);
  assert.deepEqual(items, [{ id: 1 }, { id: 2 }]);
});

test('cachedScan resumes from the high-water mark', async () => {
  const storage = fakeStorage();
  storage.setItem('test:scan', JSON.stringify({ lastScannedBlock: 199, items: [{ id: 1 }] }));
  const getLogs = fakeGetLogs({ 200: [{ id: 2 }] });
  const items = await cachedScan(scanArgs({ getLogs, storage, latestBlock: 250 }));
...
test('cachedScan falls back to a full rescan on a corrupt cache', async () => {
  ...
  const items = await cachedScan(scanArgs({ getLogs, storage, latestBlock: 150 }));
...
test('a chunk that throws leaves prior progress cached, then resumes', async () => {
  const storage = fakeStorage();
  let calls = 0;
  const getLogs = async ({ fromBlock, toBlock }: { fromBlock: number; toBlock: number }) => {
    calls++;
    if (calls === 2) throw new Error('rpc hiccup');
    return fromBlock === 100 ? [{ id: 1 }] : [{ id: 2 }];
  };
  await assert.rejects(() => cachedScan(scanArgs({ getLogs, storage, latestBlock: 25099 })), /rpc hiccup/);
  ...
  const items = await cachedScan(scanArgs({ getLogs: retryLogs, storage, latestBlock: 25099 }));
  assert.deepEqual(retryLogs.ranges, [[10100, 20099], [20100, 25099]]);
  assert.deepEqual(items, [{ id: 1 }, { id: 2 }]);
});
...
test('create-flow seed survives the next scan and dedupes against it', async () => {
  ...
  const record = { offering: ('0x' + 'aa'.repeat(20)) as `0x${string}`, projectName: 'Seeded', blockNumber: 123 };
  seedOffering(record, { factory, deployBlock: 100, storage });

  const getLogs = fakeGetLogs({}); // scan finds nothing new
  const offerings = await listOfferings({ factory, deployBlock: 100, storage, getLogs, latestBlock: 150 });
  assert.deepEqual(offerings, [record]);
  assert.equal(getLogs.ranges[0][0], 100, 'seeding does not skip unscanned history');

  seedOffering(record, { factory, deployBlock: 100, storage });
  assert.equal(JSON.parse(storage.getItem('pact:offerings:' + factory)!).items.length, 1, 'seed is idempotent');

  assert.deepEqual(findOffering(offerings, record.offering.toUpperCase().replace('0X', '0x')), record);
