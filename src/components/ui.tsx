...
export const Sub = ({ children }: { children: ReactNode }) => <span className="t-muted">{children}</span>;
...
export function Button({ variant = 'primary', className, children, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & {
...
export function TextButton({ tone, className, children, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & {
  tone?: 'danger' | 'muted';
}) {
  return (
...
// Dotted notice box.
export function Notice({ className, children }: { className?: string; children: ReactNode }) {
