...
const fakeStorage = () => {
  ...
  return { getItem: (k: string) => (map.has(k) ? map.get(k)! : null), setItem: (k: string, v: string) => void map.set(k, v) };
};

test('fragment codec round-trips a voucher', async () => {
  const { linkPrivateKey, linkKey, allocationId } = newAllocationKey();
  const voucher = { allocationId, buyerName: 'Ada Lovelace', amountCapUsdc: '125000000', linkKey };
  ...
  assert.equal(decoded.voucher.linkKey, linkKey, 'link key re-derived from the private key');
...
test('fragment decode strips a leading #', () => {
  ...
  assert.equal(decodeVoucherFragment('#' + fragment).voucher.allocationId, allocationId);
});
...
test('fragment decode rejects malformed payloads', () => {
  ...
  // Wrong version
  const payload = { v: 2, a: '0x' + '11'.repeat(32), n: 'X', c: '1', s: '0x' + '00'.repeat(65), k: '0x' + '22'.repeat(32) };
...
test('golden vector matches the checked-in fixture byte for byte', async () => {
  ...
  const checkedIn = JSON.parse(fs.readFileSync(path.join(root, 'tests', 'fixtures', 'voucher-golden.json'), 'utf8'));
