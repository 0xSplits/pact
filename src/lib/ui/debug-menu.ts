...
export function initDebugMenu({ states, getState, setState }: DebugMenuOptions): void {
  ...
  function sync() {
    panel.querySelectorAll<HTMLElement>('[data-debug-state]').forEach(button => {
  ...
  panel.addEventListener('click', e => {
    const button = (e.target as HTMLElement).closest<HTMLElement>('[data-debug-state]');
    if (!button) return;
