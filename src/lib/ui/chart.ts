...
export function drawCurve(canvas: HTMLCanvasElement, cfg: CurveChartConfig): void {
  ...
  const cmoney = (v: number) => v >= 1e6 ? '$' + (v / 1e6).toFixed(v % 1e6 === 0 ? 0 : 2) + 'M' : v >= 1e3 ? '$' + Math.round(v / 1e3) + 'K' : '$' + Math.round(v);
  ...
  if (hasSlice) {
    ...
    ctx.font = mono(26); ctx.textAlign = 'center'; ctx.fillText(slice!.label || 'You', sx((slf + slt) / 2), sy(valAt(slt)) - 18);
  }
...
export function attachCurveHover(canvas: HTMLCanvasElement, getCfg: () => CurveChartConfig | null): void {
