...
export function initDebugMenu({
  ...
  getState,
  setState,
}: DebugMenuOptions): void {
  if (!isLocalhost()) return;
  const menu = document.createElement("div");
  menu.className = "debug-menu show";
  menu.innerHTML =
    '<button class="debug-toggle" type="button">Debug</button>' +
    '<div class="debug-panel"><div class="debug-label">Offering state</div>' +
    states
      .map(
        ({ value, label }) =>
          `<button type="button" data-debug-state="${value}"><span>${label}</span><span class="debug-check" aria-hidden="true"></span></button>`,
      )
      .join("") +
    "</div>";
  ...
  function sync() {
    panel
      .querySelectorAll<HTMLElement>("[data-debug-state]")
      .forEach((button) => {
        button.classList.toggle(
          "active",
          button.dataset.debugState === getState(),
        );
      });
  }
