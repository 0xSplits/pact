...
PACT: raise small onchain rounds by selling a slice of a project's cap table — a custom liquid-split ERC-1155 (`PactToken`, token id 0, 1000 units = 100%) — along a linear bonding curve, in USDC on Base mainnet. Fully serverless: a static Vite app on Vercel with the chain as the only backend. Prototype. See `docs/` (architecture.md, contracts.md, app.md) for the system design.
